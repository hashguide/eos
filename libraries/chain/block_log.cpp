...
#warning: TODO avoid heap allocation
   ...
   uint64_t trim_data::block_pos(uint32_t n) {
      ...
      if (preamble.version >= 4) {
         uint64_t block_n_plus_1_pos = log.size();
         if (n < last_block) {
            read_buffer<uint64_t>(index.data() + index_pos + 1);
         }

         uint32_t entry_offset = read_buffer<uint32_t>(log.data() + block_n_pos);
   ...
   void block_log::smoke_test(fc::path block_dir, uint32_t interval) {
      ...
      if (interval == 0) {
         interval = std::max((td.last_block + 8 - td.preamble.first_block_num) >> 3, 1U);
      }
      for (uint32_t n = td.preamble.first_block_num; ; n += interval) {
         if (n > td.last_block)
            n = td.last_block;
